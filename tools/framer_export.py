...
Each component is emitted **self-contained**: the runtime and the stylesheet are
inlined into every file, so a component imports nothing but `react` and `framer`
- both of which Framer provides. There is no project-relative import to resolve,
which is the one thing that cannot be tested from outside Framer.
...
// Edit tools/framer_components/%s.tsx or assets/css/style.css, then run:
//     python3 tools/framer_export.py
//
// Self-contained on purpose: this file imports only `react` and `framer`, both
// of which Framer provides. Everything else - the design system stylesheet, the
// artwork and the runtime hooks - is inlined below, so there is no
// project-relative import for Framer to resolve.
...
''')
    runtime = open(os.path.join(ROOT, "tools", "framer_runtime.tsx"), encoding="utf-8").read()
    parts.append(runtime)
    shared = "".join(parts)

    src_dir = os.path.join(ROOT, "tools", "framer_components")
    total = 0
    for name in sorted(os.listdir(src_dir)):
        if not name.endswith(".tsx"):
            continue
        component = open(os.path.join(src_dir, name), encoding="utf-8").read()
        stem = os.path.splitext(name)[0]
        path = os.path.join(OUT, name)
        with open(path, "w", encoding="utf-8") as fh:
            fh.write((HEADER % stem) + shared + "\n" + component)
        size = os.path.getsize(path) / 1024
        total += 1
        print("  wrote framer/%-20s %6.1f KB" % (name, size))
    print("%d self-contained components, %d inlined assets" % (total, len(assets)))
